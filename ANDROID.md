...
1.0.0 已通过 Android 15 模拟器的六项核心测试；运行测试抽查代表性课文，全部正文与公式另有网站构建校验。系统文件选择器与打印面板尚未在各品牌实机上逐一验证。Android Lint 无错误；保留的提示包括 JavaScript 支持、固定工具版本和目标系统兼容模式，已结合本地页面白名单与调用版本判断复核。

正式包申请权限数为 0、调试标记关闭。99 个实际离线资源与源码逐项一致（网站专用 `.nojekyll` 不进入 APK）。签名使用本机保存的 RSA 3072 位密钥，APK v2/v3 校验通过；公开校验文件与 APK 一同发布，私钥及口令不在仓库中。
